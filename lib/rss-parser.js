	...
	var RSSParser = Class.extend({
		...
		_parse: function(callback) {
			...
			try {
				...
				// });
				console.log('+++++++++++++++++++++');
				this.scrapeImageFromUrlsIfNeeded(this.posts, function () {
					console.log('========_________========');
					scope.currentCallback(this);
				});
			} catch (e) {
				this.error = true;
		...
		scrapeImageFromUrlsIfNeeded: function (posts, callback) {
			...
			async.each(posts, function(post, fn) {
				...
				if ((! post.image || ! post.image.url) && post.url && ! post.url.includes('.mp3')) {
					xray(post.url, 'img', ['@src'])(function(err, srcs) {
						if (!srcs || err || srcs.length == 0) {
							completeIfNeeded();
							return
						}
		...
		imageUrlIsClean: function (url) {
			...
			while(length--) {
				if (url.toLowerCase().indexOf(subStrings[length])!= -1) {
					return false;
				}
			}
