	...
	var RSSParser = Class.extend({
		...
		parseRSSResponse: function(rootElement) {
			...
			for ( var i = 0, post; (post = posts[i]) && i < 50; i++ ) {
				...
				var descriptionElement = post.find('content,content\\:encoded');
				var realDescp = post.find('description').text();
				var author = post.find('dc\\:creator,creator').text();
		...
		parseAtomResponse: function(rootElement) {
			...
			for ( var i = 0, post; (post = posts[i]); i++ ) {
				...
				var realDescp = post.find('summary').text();
				var summary = descriptionElement.text();
