(function() {
	...
	var RSSParser = Class.extend({
		...
		checkFeedType: function(typeCallback) {
			if (this.path.includes("youtube", "y2u.be")) {
				typeCallback("youtube");
			} else {
				typeCallback("blog");
			}
		},
		...
		parseRSSResponse: function(rootElement) {
			...
			for ( var i = 0, post; (post = posts[i]) && i < 50; i++ ) {
				...
				var content = post.find('content,content\\:encoded'),
